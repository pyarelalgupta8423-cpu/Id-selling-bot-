...
# ---------- Cache ----------
_admin_cache = {"data": None, "timestamp": 0}
_CACHE_TTL = 5  # seconds

async def get_cached_admins():
    global _admin_cache
    now = datetime.utcnow().timestamp()
    if _admin_cache["data"] is None or (now - _admin_cache["timestamp"]) > _CACHE_TTL:
        _admin_cache["data"] = await db.get_admins()
        _admin_cache["timestamp"] = now
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"DB connection error in start: {e}")
        await update.message.reply_text("⚠️ Database error. Please try again later.", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    admins = await get_cached_admins()
...
# ============================================================
# BROADCAST COMMAND (moved from message_handler)
# ============================================================
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tasks = []
    for user in users:
        tasks.append(context.bot.send_message(user["user_id"], msg))
    # Send in batches of 10 to avoid flooding
    for i in range(0, len(tasks), 10):
        batch = tasks[i:i+10]
        results = await asyncio.gather(*batch, return_exceptions=True)
        success += sum(1 for r in results if not isinstance(r, Exception))
        if (i+10) % 50 == 0 or i+10 >= len(tasks):
...
# ============================================================
# SERVICE LISTINGS – ALWAYS SHOW ALL SERVICES (even if stock = 0)
# ============================================================
...
# ============================================================
# PURCHASE HANDLERS (Accounts) – with retry for race condition
# ============================================================
...
async def confirm_account_purchase(query, context, service_id):
    ...
    # Attempt purchase with retry (max 3 attempts) to avoid race condition
    account = None
    ...
    if not account:
        await query.edit_message_text(
            "❌ No accounts available or insufficient balance. Try another service.",
    ...
    # Send 2FA password separately if set
    if two_fa_password:
...
# ============================================================
# PURCHASE HANDLERS (Sessions) – with retry
# ============================================================
...
async def confirm_session_purchase(query, context, service_id):
    ...
    if not item:
        await query.edit_message_text(
            "❌ No sessions available or insufficient balance. Try another service.",
...
# -------- Admin OTP flows – FIXED: create/update account, save session, verify --------
async def admin_add_account_phone(update, context, phone):
    ...
    # Check if account exists – clear all sold/session data if reusing
    existing = await db.get_account_by_phone(phone)
    if existing:
        # Reset all fields to make it a fresh account
        await db.db.accounts.update_one(
            {"phone": phone},
            {"$set": {
        ...
        logger.info(f"Reset existing account {phone} and assigned to service {service_id}")
    else:
        await db.add_account_to_service(service_id, phone)
        logger.info(f"Created new account {phone} in service {service_id}")

    # Now proceed with login to get session
    client = TelegramClient(StringSession(), API_ID, API_HASH)
    try:
        await client.connect()
        # We always need OTP; remove dead is_user_authorized check
...
async def admin_add_account_otp(update, context, otp):
    ...
    try:
        ...
        # Account already exists (created or reset) – ensure it exists
        existing = await db.get_account_by_phone(phone)
        if not existing:
            await db.add_account_to_service(service_id, phone)
        session_string = StringSession.save(client.session)
        # Store session and ask for 2FA password immediately
        await db.save_account_session(phone, session_string)
        # Now ask for 2FA password
        await update.message.reply_text(
...
async def admin_add_account_2fa_login(update, context, password):
    ...
    try:
        ...
        if not existing:
            await db.add_account_to_service(service_id, phone)
        session_string = StringSession.save(client.session)
        # Store session and password immediately
        await db.save_account_session(phone, session_string, two_fa_password=password)  # store the 2FA password
        await update.message.reply_text(
            "✅ Logged in with 2FA. Account ready.",
...
# ============================================================
# PAYMENT FUNCTIONS (FIXED ORDER: mark verified FIRST, then add balance)
# ============================================================
...
async def verify_payment(query, context):
    ...
    # 2. Mark payment as verified in DB FIRST
    try:
        await db.verify_payment(order_id)
    except Exception as e:
        logger.error(f"Failed to mark payment verified: {e}")
        await query.answer("⚠️ Could not update payment status. Please contact support.", show_alert=True)
        return

    # 3. Now add balance to user
    try:
        await db.update_user_balance(user_id, amount)
    except Exception as e:
        logger.error(f"Failed to update balance: {e}")
        # Payment is already marked verified, but balance not added – manual intervention needed.
        await query.answer("⚠️ Payment verified but balance update failed. Contact support.", show_alert=True)
        # Still show success but with warning
        balance = await db.get_user_balance(user_id)
        await query.edit_message_text(
            f"⚠️ **Payment Verified but Balance Update Failed!**\n"
            f"Please contact support with Order ID: `{order_id}`.\n"
            f"Your current balance: ₹{format_price(balance)}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="start_back", style="primary")]]),
            parse_mode=ParseMode.MARKDOWN
        )
...
async def restore_sessions(bot):
    ...
    for acc in sold_accounts:
        ...
        if user_id and session_string:
            tasks.append(start_otp_forwarding(phone, user_id, session_string, account_id, two_fa_password, bot))
    if tasks:

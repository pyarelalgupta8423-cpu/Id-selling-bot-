...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🛒 Buy Account":
        ...
        await show_account_services(dummy, context)
        return
    ...
    if text == "🔐 Buy Session":
        class DummyQuery:
            ...
            async def answer(self, *args, **kwargs):
                pass
        dummy = DummyQuery(update.message)
        await show_session_services(dummy, context)
        return
...
async def handle_payment_callback(query, context):
    ...
    if data == "pay_custom":
        ...
        await query.edit_message_text(
            "💳 Enter amount (min ₹10, max ₹10,000):",
        ...
        if text == "/done":
            ...
            for i, user in enumerate(users):
                try:
                    ...
                    continue
            await progress.edit_text(f"✅ Broadcast complete: {success}/{len(users)}", reply_markup=get_admin_inline_keyboard(), parse_mode=ParseMode.MARKDOWN)
            context.user_data.pop("admin_state", None)
            context.user_data.pop("announce_messages", None)
        else:
            context.user_data["announce_messages"].append(update.message)
            await update.message.reply_text(f"📥 Message captured ({len(context.user_data['announce_messages'])}). Send /done to broadcast.", parse_mode=ParseMode.MARKDOWN)
        return

...
async def show_account_service_detail(query, context, service_id):
    ...
    keyboard = [
        [InlineKeyboardButton(f"✅ Buy Now - ₹{format_price(service['price'])}", callback_data=f"buy_acc_{service_id}")],
...
async def handle_account_purchase(query, context, service_id):
    ...
    if balance < price:
        ...
        return
    keyboard = [
        [InlineKeyboardButton("✅ Confirm", callback_data=f"confirm_acc_{service_id}")],
...
async def show_session_service_detail(query, context, service_id):
    ...
    text = (f"🔐 **{service['name']}**\n━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            f"📝 {service.get('description', '')}\n━━━━━━━━━━━━━━━━━━━━━━━")
    keyboard = [
        [InlineKeyboardButton(f"✅ Buy Now - ₹{format_price(service['price'])}", callback_data=f"buy_sess_{service_id}")],
...
async def handle_session_purchase(query, context, service_id):
    ...
    if balance < price:
        ...
        return
    keyboard = [
        [InlineKeyboardButton("✅ Confirm", callback_data=f"confirm_sess_{service_id}")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel_operation")]
    ]
    await query.edit_message_text(
        f"🧾 **Confirm Purchase**\nService: {service['name']}\nPrice: ₹{format_price(price)}",
...
async def confirm_session_purchase(query, context, service_id):
    ...
    keyboard = [
        [InlineKeyboardButton("🛒 Buy More", callback_data="buy_session")],
        [InlineKeyboardButton("🏠 Main Menu", callback_data="start_back")]
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)
...
async def show_payment_options(query, context):
    keyboard = [
        [InlineKeyboardButton("₹100", callback_data="pay_100"), InlineKeyboardButton("₹200", callback_data="pay_200"), InlineKeyboardButton("₹500", callback_data="pay_500")],
        [InlineKeyboardButton("₹1000", callback_data="pay_1000"), InlineKeyboardButton("Custom", callback_data="pay_custom")],
        [InlineKeyboardButton("🔙 Back", callback_data="start_back")]
    ]
    await query.edit_message_text(
        "💰 **Add Balance**\nSelect amount:",
...
# ============================================================
# ADMIN ACCOUNT SERVICES MANAGEMENT
# ============================================================
async def admin_show_account_services(query, context):
    ...
    if not services:
        await query.edit_message_text(
            "No account services.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ New", callback_data="admin_new_account_service")],
    ...
    keyboard.append([InlineKeyboardButton("➕ New Account Service", callback_data="admin_new_account_service")])
...
async def admin_edit_account_service(query, context, service_id):
    ...
    text = (f"📱 **{service['name']}**\n"
            ...
            f"📝 {service.get('description', '')}")
    keyboard = [
        [InlineKeyboardButton("➕ Add Account", callback_data=f"admin_add_acc_phone_{service_id}")],
        ...
        [InlineKeyboardButton("❌ Delete Service", callback_data=f"admin_del_acc_service_{service_id}")],
...
# ============================================================
# ADMIN SESSION SERVICES MANAGEMENT
# ============================================================
async def admin_show_session_services(query, context):
    ...
    if not services:
        await query.edit_message_text(
            "No session services.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ New", callback_data="admin_new_session_service")],
                [InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]
            ]),
            parse_mode=ParseMode.MARKDOWN
        )
    ...
    keyboard.append([InlineKeyboardButton("➕ New Session Service", callback_data="admin_new_session_service")])
    keyboard.append([InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")])
    await query.edit_message_text(
        "📋 **Session Services**",
        reply_markup=InlineKeyboardMarkup(keyboard),
...
async def admin_edit_session_service(query, context, service_id):
    ...
    text = (f"🔐 **{service['name']}**\n"
            ...
            f"📝 {service.get('description', '')}")
    keyboard = [
        [InlineKeyboardButton("➕ Add Session", callback_data=f"admin_add_sess_string_{service_id}")],
        ...
        [InlineKeyboardButton("❌ Delete Service", callback_data=f"admin_del_sess_service_{service_id}")],
...
# ============================================================
# ADMIN OTP FLOW (Add Account via Phone)
# ============================================================
async def admin_add_account_phone(update, context, phone):
    ...
    try:
        ...
        if await client.is_user_authorized():
            # Already logged in, just add account
...
# ============================================================
# ADMIN OTHER FUNCTIONS (Settings, Users, Admins, Payments)
# ============================================================
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "add_session_string":
        ...
        # Check if session string already exists (optional)
    ...
    if context.user_data.get("payment_state") == "custom_amount":
        try:
            ...
            if amount < 50 or amount > 10000:
                await update.message.reply_text("❌ Amount must be between ₹50 and ₹10,000.", parse_mode=ParseMode.MARKDOWN)
                return
            context.user_data.pop("payment_state", None)
            # Simulate a query object to reuse payment QR generation
            class FakeQuery:

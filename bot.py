...
async def confirm_account_purchase(query, context, service_id):
    ...
    # ---------- FIX: Re-fetch account to get session_string ----------
    account = await db.db.accounts.find_one({"_id": account["_id"]})
    phone = account["phone"]
    ...
    if session_string:
        asyncio.create_task(start_otp_forwarding(phone, user_id, session_string, account["_id"], two_fa_password, context))
    else:
        await query.message.reply_text(
        ...
        # Try one more time by phone lookup (in case the account was updated after purchase)
        ...
        if refreshed and refreshed.get("session_string"):
            ...
            asyncio.create_task(start_otp_forwarding(phone, user_id, refreshed["session_string"], account["_id"], two_fa_password, context))
...
# ============================================================
# OTP FORWARDING – uses the stored session string
# ============================================================
async def start_otp_forwarding(phone, user_id, session_string, account_id, two_fa_password, context):
...
# -------- Admin OTP flows (FIXED – save session) --------
async def admin_add_account_phone(update, context, phone):
    ...
    try:
        ...
        if await client.is_user_authorized():
            ...
            # Ensure account is inserted, then save session
            existing = await db.db.accounts.find_one({"phone": phone})
            if not existing:
...
async def admin_add_account_otp(update, context, otp):
    ...
    try:
        ...
        # Insert account and save session
        await db.add_account_to_service(service_id, phone)
        session_string = client.session.save()
        await db.save_account_session(phone, session_string)
        await update.message.reply_text(
...
async def admin_add_account_2fa_password(update, context, text):
    ...
    # Update the account with the 2FA password
    await db.save_account_session(phone, session_string, two_fa_password)
    service = await db.get_account_service(service_id)
    await update.message.reply_text(

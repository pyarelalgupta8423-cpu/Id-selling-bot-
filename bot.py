...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "my_account":
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💰 Add Balance", callback_data="add_balance", style="bg_success")],
    ...
    if data == "support":
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📱 Contact Support", url=f"https://t.me/{support_user}", style="bg_primary")],
                [InlineKeyboardButton("🔙 Back", callback_data="start_back")]
            ]),
            parse_mode=ParseMode.MARKDOWN
        )
...
async def show_account_service_detail(query, context, service_id):
    ...
    keyboard = [
        [InlineKeyboardButton(f"✅ Buy Now - ₹{format_price(service['price'])}", callback_data=f"buy_acc_{service_id}", style="bg_success")],
...
async def handle_account_purchase(query, context, service_id):
    ...
    if balance < price:
        ...
        return
    keyboard = [
        [InlineKeyboardButton("✅ Confirm", callback_data=f"confirm_acc_{service_id}", style="bg_success")],
...
async def confirm_account_purchase(query, context, service_id):
    ...
    keyboard = [
        [InlineKeyboardButton("🛒 Buy More", callback_data="buy_account", style="bg_primary")],
...
async def show_session_service_detail(query, context, service_id):
    ...
    text = (f"🔐 **{service['name']}**\n━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            f"📝 {service.get('description', '')}\n━━━━━━━━━━━━━━━━━━━━━━━")
    keyboard = [
        [InlineKeyboardButton(f"✅ Buy Now - ₹{format_price(service['price'])}", callback_data=f"buy_sess_{service_id}", style="bg_success")],
...
async def handle_session_purchase(query, context, service_id):
    ...
    if balance < price:
        ...
        return
    keyboard = [
        [InlineKeyboardButton("✅ Confirm", callback_data=f"confirm_sess_{service_id}", style="bg_success")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel_operation", style="bg_danger")]
    ]
    await query.edit_message_text(
        f"🧾 **Confirm Purchase**\nService: {service['name']}\nPrice: ₹{format_price(price)}",
...
async def confirm_session_purchase(query, context, service_id):
    ...
    keyboard = [
        [InlineKeyboardButton("🛒 Buy More", callback_data="buy_session", style="bg_primary")],
        [InlineKeyboardButton("🏠 Main Menu", callback_data="start_back")]
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)

# ============================================================
# PAYMENT (UPI QR) - Preset buttons removed, only custom amount
# ============================================================
...
async def verify_payment(query, context):
    ...
    if result.get("verified"):
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="start_back", style="bg_primary")]]),
            parse_mode=ParseMode.MARKDOWN
        )
...
# ============================================================
# ADMIN CALLBACKS (full implementation – keep as is)
# ============================================================
...
async def admin_show_account_services(query, context):
    ...
    if not services:
        await query.edit_message_text(
            "No account services.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ New", callback_data="admin_new_account_service", style="bg_success")],
    ...
    keyboard.append([InlineKeyboardButton("➕ New Account Service", callback_data="admin_new_account_service", style="bg_success")])
...
async def admin_edit_account_service(query, context, service_id):
    ...
    text = (f"📱 **{service['name']}**\n"
            ...
            f"📝 {service.get('description', '')}")
    keyboard = [
        [InlineKeyboardButton("➕ Add Account", callback_data=f"admin_add_acc_phone_{service_id}", style="bg_success")],
        ...
        [InlineKeyboardButton("❌ Delete Service", callback_data=f"admin_del_acc_service_{service_id}", style="bg_danger")],
...
async def admin_show_session_services(query, context):
    ...
    if not services:
        await query.edit_message_text(
            "No session services.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ New", callback_data="admin_new_session_service", style="bg_success")],
                [InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]
            ]),
            parse_mode=ParseMode.MARKDOWN
        )
    ...
    keyboard.append([InlineKeyboardButton("➕ New Session Service", callback_data="admin_new_session_service", style="bg_success")])
    keyboard.append([InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")])
    await query.edit_message_text(
        "📋 **Session Services**",
        reply_markup=InlineKeyboardMarkup(keyboard),
...
async def admin_edit_session_service(query, context, service_id):
    ...
    text = (f"🔐 **{service['name']}**\n"
            ...
            f"📝 {service.get('description', '')}")
    keyboard = [
        [InlineKeyboardButton("➕ Add Session", callback_data=f"admin_add_sess_string_{service_id}", style="bg_success")],
        ...
        [InlineKeyboardButton("❌ Delete Service", callback_data=f"admin_del_sess_service_{service_id}", style="bg_danger")],
...
async def admin_show_admins(query, context):
    ...
    text = "🔑 **Admins**\n" + "\n".join(lines) + "\n\n🔰 = Owner"
    keyboard = [
        [InlineKeyboardButton("➕ Add Admin", callback_data="admin_add_admin", style="bg_success")],
        [InlineKeyboardButton("➖ Remove Admin", callback_data="admin_remove_admin", style="bg_danger")],
        [InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)

...
from telegram.error import BadRequest  # <-- for handling "Message is not modified"
...
# ============================================================
# generate_payment_qr – works with both Update and CallbackQuery
# ============================================================
async def generate_payment_qr(source, context, amount):
    """Generate payment QR from either Update or CallbackQuery"""
    if hasattr(source, 'effective_user'):  # It's an Update
    ...
    if edit_func:
        await edit_func(final_text, reply_markup=get_payment_keyboard(order_id), parse_mode=ParseMode.MARKDOWN)
    else:
        await reply_func(final_text, reply_markup=get_payment_keyboard(order_id), parse_mode=ParseMode.MARKDOWN)

# ============================================================
# FIXED: verify_payment – handles BadRequest, prevents duplicate credits
# ============================================================
async def verify_payment(query, context):
    user_id = query.from_user.id
    order_id = query.data.split("_")[2]
    
    # Ensure it's the user's pending payment
    ...
    if payment.get("status") == "verified":
        ...
        # Remove verify button and show success message
        try:
            await query.edit_message_text(
                "✅ **This payment has already been verified.**\n\n"
                "You can check your balance via 'My Profile'.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🏠 Main Menu", callback_data="start_back", style="primary")]
                ]),
                parse_mode=ParseMode.MARKDOWN
            )
        except Exception:
    ...
    # ✅ Success – mark as verified first (to prevent duplicate processing)
    try:
    ...
    try:
        ...
        # Payment is already marked verified, but balance not credited – this is a critical error.
        # In production, you might want to alert the admin or add a retry mechanism.
        return
    ...
    # Edit message to remove verify button and show success
    try:
        await query.edit_message_text(
            f"✅ **Payment Verified!**\n"
            f"💰 ₹{format_price(amount)} added.\n"
...
# ============================================================
# ADMIN CALLBACKS (full – same as previous)
# ============================================================
...
# -------- Admin Service Management (unchanged) --------
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("payment_state") == "custom_amount":
        try:
            ...
            # Pass the update directly (no FakeQuery needed)
...
# ============================================================
# MAIN – with Flask and proper event loop
# ============================================================
async def main():

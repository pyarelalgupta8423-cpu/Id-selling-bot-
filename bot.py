...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------- ADMIN PANEL BUTTON (always works) ----------
    ...
    # ---------- MAIN MENU BUTTONS ----------
    if text == "🛒 Buy Account":
        await show_account_services_from_message(update, context)
        return
    ...
    if text == "💰 Add Balance" or text == "💰 Wallet":
...
# ============================================================
# SERVICE LISTING – 3 SEPARATE BUTTONS PER SERVICE (Account)
# ============================================================
async def show_account_services_from_message(update, context):
    ...
    text = "📱 **Available Telegram Services**\n━━━━━━━━━━━━━━━━━━━━━━━\n"

    keyboard = []
    for s in current:
        sid = str(s["_id"])
        avail = await db.get_account_service_available_count(sid)
        country_text = s['name'][:15]
        price_text = f"₹{format_price(s['price'])}"
        stock_text = f"[{avail}]"
        # Three buttons in one row
        row = [
            InlineKeyboardButton(country_text, callback_data=f"acc_service_{sid}", style="primary"),
            InlineKeyboardButton(price_text, callback_data=f"acc_service_{sid}", style="primary"),
            InlineKeyboardButton(stock_text, callback_data=f"acc_service_{sid}", style="primary")
        ]
        keyboard.append(row)
...
# ============================================================
# SERVICE LISTING – 3 SEPARATE BUTTONS PER SERVICE (Session)
# ============================================================
async def show_session_services_from_message(update, context):
    ...
    for s in current:
        ...
        stock_text = f"[{avail}]"
        row = [
            InlineKeyboardButton(name_text, callback_data=f"sess_service_{sid}", style="primary"),
            InlineKeyboardButton(price_text, callback_data=f"sess_service_{sid}", style="primary"),
            InlineKeyboardButton(stock_text, callback_data=f"sess_service_{sid}", style="primary")
        ]
        keyboard.append(row)
...
# ============================================================
# SERVICE DETAIL – Back button fixed
# ============================================================
...
async def confirm_account_purchase(query, context, service_id):
    ...
    text = (f"✅ **Purchase Successful!**\n━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            f"Click 'Logout & Remove' when done to revoke session.")
    keyboard = [
        [InlineKeyboardButton("🛒 Buy More", callback_data="buy_account", style="primary")],
        [InlineKeyboardButton("🔴 Logout & Remove", callback_data=f"logout_acc_{account['_id']}", style="danger")],
        [InlineKeyboardButton("🏠 Main Menu", callback_data="start_back")]
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)

    if session_string:
        asyncio.create_task(start_otp_forwarding(phone, user_id, session_string, account["_id"]))

# ============================================================
# SESSION DETAIL – Back button fixed
# ============================================================
...
# ============================================================
async def start_otp_forwarding(phone, user_id, session_string, account_id):
...
# ============================================================
# ADMIN INPUT HANDLER (for text inputs)
# ============================================================
async def handle_admin_input(update, context, text):
    ...
    if state == "new_acc_service_desc":
        ...
        await update.message.reply_text(f"✅ Account Service '{name}' created.", reply_markup=get_admin_inline_keyboard(), parse_mode=ParseMode.MARKDOWN)
        context.user_data.pop("admin_state", None)

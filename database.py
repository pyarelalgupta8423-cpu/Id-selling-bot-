...
class Database:
    ...
    async def add_admin(self, user_id: int) -> bool:
        result = await self.db.users.update_one({"user_id": user_id}, {"$set": {"is_owner": True, "updated_at": datetime.utcnow()}})
        return result.modified_count > 0

    async def remove_admin(self, user_id: int) -> bool:
        if user_id == int(os.getenv("OWNER_ID")):
            return False
        result = await self.db.users.update_one({"user_id": user_id}, {"$set": {"is_owner": False, "updated_at": datetime.utcnow()}})
        return result.modified_count > 0
